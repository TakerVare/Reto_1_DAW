document.addEventListener('DOMContentLoaded', function () {
    ...
    // Función para crear el contenido de un slider específico
    function createSliderContent(sliderId, text) {
        const slider = document.getElementById(sliderId);
        if (!slider) return; // Comprobación de seguridad

        const sliderContent = `<div class="text-slider-item"><span>${text}</span><span class="separator">•</span></div>`;

        // Calcular cuántas repeticiones necesitamos para llenar más de 2 veces el ancho de la pantalla
        const screenWidth = window.innerWidth;
        const contentWidth = text.length * 50 + 50; // Ancho aproximado basado en la longitud del texto
        const repetitions = Math.ceil((screenWidth * 2) / contentWidth) + 5; // Añadimos extras para asegurar el seamless

        // Crear contenido repetido
        let content = '';
        ...
        slider.innerHTML = content + content; // Duplicamos todo para la animación
    }
    ...
    function initializeAllSliders() {
        ...
        // Para cada slider, generar su contenido específico
        sliders.forEach(slider => {
            const sliderId = slider.id;
            const sliderText = slider.getAttribute('data-content');
            createSliderContent(sliderId, sliderText);
        });
    }
    ...
    window.addEventListener('resize', initializeAllSliders);

    // Gestión de categorías (si están presentes)
